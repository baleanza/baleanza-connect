...
// === ДОБАВЛЕНИЕ ОПЛАТЫ (FIXED ENDPOINT & DEBUG) ===
export async function addExternalPayment(wixOrderId, amountStr, currency, createdDate) {
    ...
    // Структура для endpoint 'add-payment' требует массива 'payments'
    const payload = {
        payments: [
            {
                amount: { 
                ...
                type: "OFFLINE", 
                channel: "OTHER", // Можно заменить на 'POINT_OF_SALE' или 'WEB' если OTHER не сработает
    ...
    try {
        ...
        const text = await res.text(); // Читаем ответ текстом для логов
