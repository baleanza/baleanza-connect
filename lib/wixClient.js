...
// Изменена на export для целей дебага (шаг из предыдущей итерации)
      ...
      const res = await fetch(`${WIX_API_BASE}/stores/v1/products/query`, {
        ...
        body: JSON.stringify({
          query: {
            limit: limit,
            skip: skip,
            // Обязательно запрашиваем variants
            fields: ["variants", "id", "sku", "priceData", "stock"] 
          }
        })
      });
  ...
  allProducts.forEach(p => {
    ...
    if (p.variants && p.variants.length > 0) {
      p.variants.forEach(v => {
        ...
        // КЛЮЧЕВОЙ ОБЪЕКТ: v.variant содержит SKU, PriceData и т.д.
        const variantDetails = v.variant; 
        if (!variantDetails) return;
        
        // ИСПРАВЛЕНИЕ: Получаем SKU из variantDetails
        const vSku = variantDetails.sku ? String(variantDetails.sku).trim() : '';
        
        if (targetSkus.has(vSku)) {
          // Stock Data: в V1 не всегда присутствует в variantDetails, поэтому ищем везде
          const stockData = variantDetails.stock || v.stock || {}; 
          
          // ИСПРАВЛЕНИЕ: Цену ищем в variantDetails
