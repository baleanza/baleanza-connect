...
// F: Units
  ...
  rows.forEach((row) => {
    ...
    if (wixItem) {
        // Устанавливаем availability: 'Є в наявності' или 'Немає в наявності'
        const isAvailable = wixItem.inStock === true;
        offerData.availability = isAvailable ? 'Є в наявності' : 'Немає в наявності';
    } else if (sku) {
        // Если SKU есть в таблице, но не найден в Wix, считаем, что товара нет (как в monomarket-stock)
    ...
    headers.forEach((header, colIndex) => {
      ...
      // Пропускаем, если мы уже установили 'availability' через Wix (wixItem != null), 
      // и текущий заголовок тоже соответствует 'availability'.
