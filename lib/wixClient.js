...
export async function addExternalRefundTransaction(wixOrderId, amountStr, currency, createdDate) {
    ...
    const positiveAmount = parseFloat(amountStr); // Используем положительную сумму

    const payload = {
        payments: [
            {
                amount: { 
                    amount: String(positiveAmount.toFixed(2)), // ПОЛОЖИТЕЛЬНАЯ СУММА
                    currency: currency 
                },
                transactionDate: transactionDate,
                regularPaymentDetails: {
                    paymentMethod: "Monomarket Refund", 
                    status: "REFUNDED" // Оставляем статус "Возвращено"
                }
            }
        ]
    };
    ...
    console.log(`Adding REFUND transaction with POSITIVE amount for order ${wixOrderId}, amount: ${positiveAmount.toFixed(2)}`);
