...
export async function updateWixOrderDetails(wixOrderId, updates) {
  ...
  try {
      ...
      if (res.ok) {
        ...
        return true; // Добавлено, чтобы ловить успех
      }
      return false;
  } catch (e) {
...
// Этот метод использует GET /ecom/v1/payments/orders/{id}, который вернул 200.
export async function getWixPaymentDetails(orderId) {
    try {
        ...
        // Структура, подтвержденная дебагом: { orderTransactions: { payments: [...] } }
        if (data.orderTransactions && Array.isArray(data.orderTransactions.payments)) {
            // Ищем первую оплату со статусом APPROVED (или null, если не найдена)
        ...
        paymentRefunds: [
            {
                paymentId: paymentId,
                amount: amountStr,
                externalRefund: true // Пометка, что возврат проводится внешней системой
            }
        ]
    };
...
// === НЕИСПОЛЬЗУЕМЫЕ/УСТАРЕВШИЕ ФУНКЦИИ (ОСТАВЛЕНЫ ДЛЯ СОВМЕСТИМОСТИ) ===

// Оставляем только GET-метод, но он не находит наши External Payments, поэтому бесполезен для FULFILLED
