...
export async function getWixOrderFulfillments(wixOrderId) {
    try {
        const res = await fetch(`${WIX_API_BASE}/ecom/v1/fulfillments/query`, {
            method: 'POST',
            headers: getHeaders(),
            body: JSON.stringify({
                query: {
                    filter: {
                        "orderId": { "$eq": wixOrderId }
                    },
                    fields: ["id", "lineItems", "trackingInfo", "status", "createdDate"]
                }
            })
        });
        ...
        if (!res.ok) {
            ...
            console.error(`Wix Fulfillments Query Error (ID: ${wixOrderId}): ${errorText}`);
            if (res.status >= 500) throw new Error(`Wix API error during fulfillment query: ${errorText}`);
            return [];
        }
