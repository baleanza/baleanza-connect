...
// === ДОБАВЛЕНИЕ ОПЛАТЫ (Исправлено по документации) ===
export async function addExternalPayment(wixOrderId, amount, currency, createdDate) {
    // Формируем дату оплаты (сейчас или дата из муркита)
    const transactionDate = createdDate ? new Date(createdDate).toISOString() : new Date().toISOString();
    ...
    const payload = {
        payment: {
            ...
            type: "OFFLINE", // Оплата вне Wix (карта на сайте муркит/наложка)
            channel: "OTHER",
            transactionDate: transactionDate
        }
    };

    console.log(`Adding PAYMENT for order ${wixOrderId}: ${amount} ${currency}`);
...
// ... Остальные функции (fetchAllProducts, getProductsBySkus, etc.) оставляем как были ...
